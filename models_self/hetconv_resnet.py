'''
具有异构卷积核的resnet（将基本块中的标准卷积替换为异构卷积）
'''

import torch.nn as nn
import torch
from models_self.HetConv import HetConv, Efficient_HetConv


class BasicBlock(nn.Module):
    '''
    1. 基本块，非瓶颈结构，没有升维和降维操作
    2. 包含两层卷积层，每一层的卷积核尺寸都是3×3和 1×1，第二层步长固定为1（保持输出尺寸不变）
    3. 每一层之后都进行批归一化
    '''

    expansion = 1  #通道扩展倍数（若为瓶颈结构，该值大于1）

    def __init__(self, in_channel, out_channel, stride=1, downsample=None, **kwargs):
        super(BasicBlock, self).__init__()
        # 第一层
        #print('创建第一层异构卷积...')
        # self.conv1 = HetConv(in_channels=in_channel, out_channels=out_channel, P=out_channel,
        #                      kernel_size=3, stride=stride, bias=False)  #创建异构卷积层（P=N，N为滤波器的个数)
        self.conv1 = Efficient_HetConv(in_channels=in_channel, out_channels=out_channel, P=min(out_channel, in_channel),
                             kernel_size=3, stride=stride, bias=False)  # 创建异构卷积层（P=N，N为滤波器的个数)
        self.bn1 = nn.BatchNorm2d(out_channel)  # 批归一化实例
        self.relu = nn.ReLU()  # 激活实例
        # 第二层
        #print('创建第二层异构卷积...')
        # self.conv2 = HetConv(in_channels=out_channel, out_channels=out_channel, P=out_channel,
        #                      kernel_size=3, stride=1, bias=False)
        self.conv2 = Efficient_HetConv(in_channels=out_channel, out_channels=out_channel, P=min(out_channel, in_channel),
                             kernel_size=3, stride=1, bias=False)  # 创建异构卷积层（P=N，N为滤波器的个数)
    ...
    def forward(self, x):
        ...
        #print('identity.size:', identity.shape)
